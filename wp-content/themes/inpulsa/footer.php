<footer class="container-fluid pt-5 d-flex flex-column justify-content-center align-items-center">
<div class="row w-100">
           <div class="col-12 col-sm-12 col-md-4 col-lg-4 d-flex flex-column justify-content-center align-items-center align-items-lg-start">
               <img src="../../../gravit/online.png" alt="online" class="img-fluid">
            </div>
            <div class="col-12 col-sm-12 col-md-4 col-lg-4 order-first order-sm-first order-md-1 order-lg-1 icones d-flex justify-content-center align-items-center">
                <a href="#"> <i class="fab fa-facebook-square"></i></a>
                <a href="#"> <i class="fab fa-twitter"></i></a>
                <a href="#"> <i class="fab fa-linkedin"></i></a>
            </div>
            <div class="col-12 col-sm-12 col-md-4 col-lg-4 order-last d-flex flex-column justify-content-center align-items-center align-items-lg-end">
                <img src="../../../gravit/gen.png" alt="gen" class="img-fluid">
            </div>
        </div>
        <div class="row w-100">
            <div class="col-12 col-sm-12 col-md-4 col-lg-4 d-flex flex-column justify-content-center align-items-center align-items-lg-start ">
                <h5>À Propos</h5>
                <p class="text-apropos">Si la formation t'intéresse n'hésite pas à <a href="contact" class="lien-hover">nous contacter</a> pour la prochaine session.</p>
            </div>
            <div class="col-12 col-sm-12 col-md-4 col-lg-4 d-flex flex-column justify-content-center align-items-center">
                <img src="../../../gravit/access.png" alt="logo-acs" class="img-fluid">
            </div>


            <div class="col-12 col-sm-12 col-md-4 col-lg-4 d-flex flex-column justify-content-center align-items-center align-items-lg-end">
                <a href="http://localhost:8080/mentions-legales/" class="lien-hover">Mentions légales</p>
                <a href="http://localhost:8080/rgpd/" class="lien-hover">RGPD</p>
            </div>
        </div>
    
</footer>
<div class="copyright d-flex justify-content-center align-items-center">
    <p>Tout droits réservés ©2019 | fait avec <i class="far fa-heart"></i> par Inpulsa Team</p>
</div>  
    <?php wp_footer(); ?>
</body>
</html>